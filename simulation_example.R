...
monitor_estimate = data.frame(
  ...
  # probability of bycatch event by haul and metier. One column per metier, hardcoded, in p.bycatch matrix
  p_bycatch_1 = NA,
  p_bycatch_2 = NA,
  ...
  # Note: unclear. Values from an unclear variable, pmonitor_boatTRUE, are assigned to pmonitor. Is pmonitor_boatTRUE same as p_monitor_boat, proportion of hauls monitored for each vessel?
  pmonitor = NA,
  
  # Note: unclear. Values from an unclear variable, p_monitor_boat_boatTRUE, are assigned to p_monitor_boat. Is pmonitor_boat_boatTRUE same as p_monitor_boat, proportion of hauls monitored for each vessel?
  p_monitor_boat = NA,
  
  # boat_samp (samp_vessel)
  # if FALSE: sampling occurs at the fishing event level (haul); if TRUE: first sample vessels to be monitored, then sample hauls
  boat_samp = NA,
  
  # bymetier (strat_metier); is sampling stratified by metier?
  bymetier = NA,
  
  # p_monitor_metier: proportion of monitoring allocated to (each?) métier
  p_monitor_metier = NA,
  
  # sum number of bycatch individuals divided by number of hauls = number of rows in 'fishing'; see bewlow
  BPUE_real = NA,
...
for (y in 1:100) {
  ...
  # create fishing year for single values of b and m 
  fishing = make_fishing_year_metier(
    
    # select a row from p.bycatch 
    # Note: is it good practice to assign the selection to an object with the same name?
  ...
  # Note: nrow easier than dim(...)[1]
  BPUE_real = sum(fishing$nbycatch) / dim(fishing)[1]
  print("fishing done")
  flush.console()
  
  
  # for each value in pmonitor_boatTRUE
  # Note: unclear argument
  ...
  for (pm.b in 1:length(pmonitor_boatTRUE)) {
    ...
    # for each value in p_monitor_boat_boatTRUE
    # Note: unclear argument
      ...
      for (p_m_m in 1:dim(p_monitor_metier)[1]) {
        ...
        if(bymetier == FALSE) {
          ...
          temp = data.frame(
            ...
            # p_bycatch_
            # one column for each of the metiers, hard-coded 
